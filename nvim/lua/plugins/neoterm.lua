return {
  {
    ...
    config = function()
      ...
      -- bind("n", "<A-S-i>", function()
      --   require("neoterm").open({ position = "bottom", height = 0.4 })
      -- end) -- Open second terminal (normal mode)
      -- bind("t", "<A-S-i>", "<C-\\><C-n>:NeotermToggle<CR>") -- Toggle second terminal (terminal mode)
      -- bind("i", "<A-S-i>", "<Esc>:NeotermToggle<CR>") -- Toggle second terminal (insert mode)
    end,
  },
}
